...
def find_class_title(str):
    ...
    for i in range(len(str)-2):
        if "國高美餐應廣英子訊商觀".find(str[i]) > -1:
            if "一二三際".find(str[i+1]) > -1:
                if "0123456789".find(str[i+2]) > -1:
                    return True
    return None
...
# 取得當前資料夾下所有 .docx 檔案
docx_files = [file.name for file in Path.cwd().glob("*.docx")]

# 為每個檔案加上 prefix "result"
result_files = [f"result_{file.stem}.pptx" for file in Path.cwd().glob("*.docx")]


parsing_doc(docx_files[0] , result_files[0], 't.pptx')
